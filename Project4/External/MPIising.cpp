/* 
		MORTENS CODE – USED ON ABEL TO GET REASONABLE RESULTS
...
int main(int argc, char* argv[])
{
	char *outfilename;
	long idum;
	int **spin_matrix, mcs, my_rank, numprocs;
	int n_spins[] = {40, 60, 80, 100};
	...
	mcs = 1e5;  initial_temp = 2.0; final_temp = 2.3; temp_step = 0.01;
	/*
	...
	*/
	...
	for (int y = 0; y < 4; y++){
		...
		spin_matrix = (int**) matrix(n_spins[y], n_spins[y], sizeof(int));

		for ( double temperature = initial_temp; temperature <= final_temp; temperature+=temp_step){
			//    initialise energy and magnetization 
			E = M = 0.;
			// initialise array for expectation values
			initialize(n_spins[y], spin_matrix, E, M);
			...
			for (int cycles = myloop_begin; cycles <= myloop_end; cycles++){
				Metropolis(n_spins[y], idum, spin_matrix, E, M, w);
			...
			// print results
			if ( my_rank == 0) {
				output(n_spins[y], mcs, temperature, total_average);
			}
		}
...
void output(int n_spins, int mcs, double temperature, double *total_average)
{
	...
	ofile << setw(15) << setprecision(8) << Mabstotal_average/n_spins/n_spins << endl << endl;
} // end output function
